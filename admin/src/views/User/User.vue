<template>
    <div class="user-container" v-loading="state.loading">
        ...
        <div class="content">
                    ...
                    <el-table-column fixed="right" label="操作" width="120">
                        <template #default>
                            <div class="but">
                                ...
                                <el-select v-model="state.opeate" class="m-2" placeholder="Select" size="large">
                                    <el-option v-for="item in state.userOpeates" :key="item.value" :label="item.label"
                                        :value="item.value" />
                                </el-select>
                            </div>
                        </template>
            ...
            <div class="pager">
                <el-pagination background layout="prev, pager, next" v-model:current-page="state.currentPage"
                    :total="state.total" @update:current-page="changePage" />
            </div>
        </div>
...
async function getUser(page: number, limit: number) {
    const user = await fetchAllUser(page, limit) as unknown as ResponseWithCount;
