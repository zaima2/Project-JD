    ...
    <div class="miaosha-predictor-container">
        ...
        <div class="footer">
            <div class="timer-predictor">
               <span class="time">{{ state.startTime.hour ? state.startTime.hour.toString().padStart(2,"0") : "00" }}:{{ state.startTime.min ? state.startTime.min.toString().padStart(2,"0") : "00" }}</span> 点场,距结束
            </div>
            <div class="timer-left">
                <div class="hour">{{ state.endTime.hour ? state.endTime.hour.toString().padStart(2,'0') : "00" }}</div>
                <span>:</span>
                <div class="min">{{ state.endTime.min ? state.endTime.min.toString().padStart(2,'0') : "00" }}</div>
                <span>:</span>
                <div class="sec">{{ state.endTime.sec ? state.endTime.sec.toString().padStart(2,'0') : "00" }}</div>
            </div>
        </div>
    </div>
</template>
...
watchEffect(()=>{
    ...
    state.endTime.sec = Math.floor(parseSeconds - (state.endTime.hour * 60 * 60 ,state.endTime.min * 60));
